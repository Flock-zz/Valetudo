...
const root = protobuf.Root.fromJSON(require("./schema3090.json"));
...
const INTENSITY_STATE_TYPE = IntensityStateAttribute.TYPE;
const INTENSITY_STATE_VALUE = IntensityStateAttribute.VALUE;
...
const FAN_SPEEDS = {
    [INTENSITY_STATE_VALUE.OFF]: 0,
    [INTENSITY_STATE_VALUE.LOW]: 1,
    [INTENSITY_STATE_VALUE.MEDIUM]: 2,
    [INTENSITY_STATE_VALUE.HIGH]: 3,
};
...
class DeviceStatus {
    ...
    getFanSpeedState() {
        return new IntensityStateAttribute({
            type: INTENSITY_STATE_TYPE.FAN_SPEED,
...
class Map {
    ...
    update(data) {
        ...
        for (let x = 0; x < size.x; x++) {
            for (let y = 0; y < size.y; y++) {
                ...
                if (point === 255) {
                    walls.push(x, y);
                } else if (point !== 0 && point !== undefined) {
                    floors.push(x, y);
                }
            }
        }
        ...
        this.floors.calculateDimensions(); //@TODO: It is not needed to be done in every update
        this.walls.pixels = walls;
        this.walls.calculateDimensions(); //@TODO: It is not needed to be done in every update
    ...
    updateChargerPosition(chargerInfo) {
        ...
        //this.charger.points = [charger.x * 5, charger.y * 5]; // This brokes the map exporter
        this.charger.points = [charger.x, charger.y];
    ...
    updateRobotPosition(robotInfo) {
        ...
        // hardcoded pixel size
        // must be fixed in mmToCanvasPx function
        //this.robot.points = [robot.x * 5, robot.y * 5];  // This brokes the map exporter
        this.robot.points = [robot.x, robot.y];
        this.robot.metaData.angle = angle;
	      //this.path.points.push(robot.x * 5, robot.y * 5);  // This brokes the map exporter
        this.path.points.push(robot.x, robot.y);
    }
}
...
class CecotecConga3090 extends ValetudoRobot {
    constructor(options) {
        super(options);
        ...
        this.map = new Map();
        this.timers = null;
        this.state.map = this.map.valetudoMap;
        this.handlers = {
            ...
            //DEVICE_EVENT_REPORT_CLEANMAP: this.handleMap1.bind(this),
        ...
        this.registerCapability(new capabilities.CecotecFanSpeedControlCapability({
            robot: this,
            presets: Object.keys(FAN_SPEEDS).map(k => new IntensityPreset({name: k, value: FAN_SPEEDS[k]}))
        }));
    ...
    async handleBatteryLevel(message) {
        ...
        this.state.upsertFirstMatchingAttribute(this.deviceStatus.getBatteryState());
    ...
    async handleDeviceStatus(message) {
        ...
        this.state.upsertFirstMatchingAttribute(this.deviceStatus.getBatteryState());
        this.state.upsertFirstMatchingAttribute(this.deviceStatus.getStatusState());
        this.state.upsertFirstMatchingAttribute(this.deviceStatus.getFanSpeedState());
        this.state.upsertFirstMatchingAttribute(this.deviceStatus.getLatestCleanupArea());
        this.state.upsertFirstMatchingAttribute(this.deviceStatus.getLatestCleanupDuration());
